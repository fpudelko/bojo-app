...
const alert = (n: Partial<GameAlert> = {}): GameAlert => ({
  id: 'a1', userId: 'u1', sports: [], daysOfWeek: [], lat: 51.1, lng: 17.03, radiusKm: 25,
...
describe('MojeAlerty — dom alertów w profilu', () => {
  ...
  it('wiersz mówi, czego alert pilnuje — sport, miejsce, promień', async () => {
    alerty.mockResolvedValue([alert({ sports: ['piłka nożna'], cityLabel: 'Wrocław' })]);
    render(<MojeAlerty />);
  ...
  it('WYŁĄCZONY alert zostaje na liście, a nie znika', async () => {
    ...
    alerty.mockResolvedValue([alert({ sports: ['siatkówka'], cityLabel: 'Poznań', isActive: false })]);
    render(<MojeAlerty />);
  ...
  it('każdy alert ma własny przełącznik i własne kasowanie', async () => {
    alerty.mockResolvedValue([
      alert({ id: 'a1', sports: ['piłka nożna'], cityLabel: 'Wrocław' }),
      alert({ id: 'a2', sports: ['siatkówka'], cityLabel: 'Poznań' }),
    ]);
    render(<MojeAlerty />);
