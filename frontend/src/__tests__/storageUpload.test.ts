...
describe('uploadObrazek — walidacja', () => {
  // Obie asercje odrzucają PRZED siecią, więc nie potrzebują atrapy Supabase.
  it('odrzuca plik większy niż 5 MB', async () => {
    await expect(uploadObrazek('turniej-media', 'x', plikFake(5 * 1024 * 1024 + 1, 'image/png')))
      .rejects.toThrow('Maksymalny rozmiar: 5 MB');
  });

  it('odrzuca plik, który nie jest image/*', async () => {
    await expect(uploadObrazek('turniej-media', 'x', plikFake(100, 'application/pdf')))
