...
function MojeGryContent() {
  ...
  // BEZ FILTRÓW. Stały tu dwa i oba wyleciały (2026-08-24):
  //
  // • „Nieprzeczytane" — nieprzeczytane wiadomości mają własne, mocniejsze
  //   wejście (zakładka „Rozmowy" z chmurką), a różowa plakietka na karcie
  //   i tak mówi, w którym meczu ktoś pisał;
  // • „Brakuje graczy" — po podziale na „Grasz"/„Organizujesz" mecze bez
  //   kompletu i tak stoją w kupie w „Organizujesz", a na każdej karcie widać
  //   „N wolnych miejsc". Filtr zawężał listę, która jest już pogrupowana.
  //
  ...
  const grasz = playing.filter((r) => r.relation.status === 'playing');
  const organizujesz = playing.filter((r) => r.relation.isOrganizer && r.relation.status !== 'playing');
  ...
  const pozostale = playing.filter((r) => r.relation.status !== 'playing' && !r.relation.isOrganizer);
