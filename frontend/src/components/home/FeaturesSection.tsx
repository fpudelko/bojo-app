...
function FeatureRow({ feature, reverse }: { feature: (typeof FEATURES)[number]; reverse: boolean }) {
  ...
  return (
    <div
      ref={ref}
      className={[
        'grid grid-cols-1 items-center gap-10 lg:grid-cols-2 lg:gap-16 transition-all duration-700 ease-out',
    ...
    >
      <div className="relative mx-auto w-full max-w-xs sm:max-w-sm">
        <Image
          ...
          className="w-full select-none drop-shadow-[0_24px_48px_rgba(11,52,32,0.25)]"
        />
      </div>
      <div className="max-w-lg">
        ...
        <p className="mt-3 leading-relaxed text-slate-600">{feature.body}</p>
      </div>
    </div>
  );
}
