...
function MarketingHero() {
  ...
  return (
    <section className="hero-surface relative overflow-hidden text-white">
      ...
      <div className="relative mx-auto grid max-w-6xl grid-cols-1 items-center gap-10 px-4 pb-16 pt-14 lg:grid-cols-[1.1fr_0.9fr] lg:gap-12 lg:pb-24 lg:pt-20">
...
function PersonalizedHero() {
  ...
  const today = new Date().toISOString().split('T')[0];
  const todayOpen = openEvents.filter((e) => e.date === today);
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-medium text-white/70 mb-1">
                {today.split('-').reverse().slice(0,2).join('.')} · {todayOpen.length > 0 ? `${todayOpen.length} ${todayOpen.length === 1 ? 'gra dziś' : 'gry dziś'}` : 'brak gier dziś'}
              </p>
          ...
          <div className="mt-5 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <button
              ...
              className={[
                ...
                activeSport === '' ? 'border-white bg-white text-primary-700' : 'border-white/30 bg-white/10 text-white hover:bg-white/20',
              ].join(' ')}
            >
              Wszystkie
            </button>
            {SPORT_CHIPS.map(({ sport, emoji, label }) => (
              <button
                ...
                className={[
                  'shrink-0 inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors whitespace-nowrap',
                  activeSport === sport ? 'border-white bg-white text-primary-700' : 'border-white/30 bg-white/10 text-white hover:bg-white/20',
                ].join(' ')}
              >
                {emoji} {label}
              </button>
        ...
        <div>
          ...
          {loading ? (
            ...
            <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-8 text-center">
              ...
              <p className="text-sm font-medium text-slate-600 mb-4">Brak otwartych gier w tej chwili</p>
              <button
                onClick={() => setShowAlert(true)}
