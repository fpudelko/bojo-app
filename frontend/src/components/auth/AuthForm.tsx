...
export default function AuthForm({ next, onSuccess }: Props) {
  ...
  return (
    <div>
      ...
      {/* Google — locked in in-app browsers, normal otherwise */}
      {mode !== 'reset' && (
        <>
          {inApp?.isInApp ? (
            ...
            <button
              ...
              className="mt-5 flex w-full items-center justify-center gap-2.5 rounded-xl border border-slate-300 bg-white py-3 text-sm font-semibold text-ink transition-colors hover:bg-slate-50"
            >
          ...
          <div className="my-5 flex items-center gap-3">
            ...
            <span className="text-xs font-medium uppercase tracking-wider text-slate-400">albo e-mailem</span>
            <span className="h-px flex-1 bg-slate-200" />
          </div>
        </>
      )}
...
interface Props {
  /** Where to send the user after a successful password login. */
  next?: string;
  /** Called after a successful in-place login (e.g. to close a modal). */
  onSuccess?: () => void;
}
...
export default function AuthForm({ next, onSuccess }: Props) {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Nie udało się rozpocząć logowania Google.');
    }
  };
  ...
  const submitLabel =
    ...
    : mode === 'signup' ? 'Załóż konto'
    : mode === 'magic' ? 'Wyślij link logowania'
    : 'Wyślij link resetu';

  // Success screen for email-dispatch modes
  if (info && (mode === 'magic' || mode === 'reset' || mode === 'signup')) {
    return (
      <div className="text-center">
        <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-green-50">
        ...
        <button
          ...
          className="mt-6 inline-flex items-center gap-1.5 text-sm font-medium text-primary-700 hover:text-primary-800"
        >
          <ArrowLeft className="h-4 w-4" /> Wróć do logowania
        </button>
      </div>
  ...
  return (
    <div>
      ...
      {/* Google — always available, primary path */}
      {mode !== 'reset' && (
        <>
          <button
            type="button"
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        {mode === 'signup' && (
          <div className="relative">
            ...
            <input
              ...
              placeholder="Imię lub pseudonim (opcjonalnie)"
              autoComplete="name"
              maxLength={40}
              className={inputCls}
            />
        ...
        {info && (
          <p className="rounded-lg bg-green-50 border border-green-200 px-3 py-2 text-sm text-green-700">{info}</p>
        )}
      ...
      <div className="mt-5 space-y-2 text-center text-sm">
        {mode === 'signin' && (
          <>
            <p className="text-slate-500">
              Nie masz konta?{' '}
              <button onClick={() => switchMode('signup')} className="font-semibold text-primary-700 hover:text-primary-800">
                Załóż je
              </button>
            </p>
            <button onClick={() => switchMode('magic')} className="text-xs font-medium text-slate-500 hover:text-primary-700">
        ...
        {mode === 'signup' && (
          <p className="text-slate-500">
            Masz już konto?{' '}
            <button onClick={() => switchMode('signin')} className="font-semibold text-primary-700 hover:text-primary-800">
              Zaloguj się
            </button>
          </p>
        )}
        {(mode === 'magic' || mode === 'reset') && (
          <button
            onClick={() => switchMode('signin')}
            className="inline-flex items-center gap-1.5 font-medium text-slate-500 hover:text-primary-700"
          >
            <ArrowLeft className="h-4 w-4" /> Wróć do logowania
          </button>
        )}
