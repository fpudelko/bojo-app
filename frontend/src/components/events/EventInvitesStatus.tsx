        ...
        {sorted.map((inv) => {
          ...
          return (
            <li key={inv.id} className="flex items-center gap-2.5 py-2">
              {inv.avatarUrl
                ? <img src={inv.avatarUrl} alt="" className="h-7 w-7 shrink-0 rounded object-cover" />
                : (
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded bg-primary-100 text-xs font-semibold text-primary-700">
                    {inv.name.charAt(0).toUpperCase()}
                  </span>
                )}
              <span className="min-w-0 flex-1 truncate text-sm text-ink">{inv.name}</span>
              <span className={`inline-flex shrink-0 items-center gap-1 rounded px-2 py-0.5 text-[11px] font-semibold ${cls}`}>
