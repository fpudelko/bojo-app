...
function ResultSummary({ result }: { result: MatchResult }) {
  ...
  if (!rd) {
    return (
      <div className="text-center py-2 mb-4">
        <p className="text-3xl font-bold text-gray-900 tracking-tight">{result.scoreA} — {result.scoreB}</p>
  ...
  if (rd.type === 'goals') {
    return (
      <div className="text-center py-2 mb-4">
        <p className="text-3xl font-bold text-gray-900 tracking-tight">{rd.scoreA} — {rd.scoreB}</p>
        <p className="text-xs text-gray-400 mt-1">Drużyna A · Drużyna B</p>
        {rd.scorers && rd.scorers.length > 0 && (
          <p className="text-xs text-gray-500 mt-1">{rd.scorers.length} bramka{rd.scorers.length > 1 ? 'rzy' : 'rz'}</p>
        )}
      </div>
    );
  }
  if (rd.type === 'volleyball') {
    return (
      <div className="py-2 mb-4">
        <p className="text-3xl font-bold text-gray-900 tracking-tight text-center">{rd.setsA} — {rd.setsB} <span className="text-base font-normal text-gray-400">sety</span></p>
        <div className="flex justify-center gap-2 mt-2 flex-wrap">
          {rd.sets.map((s, i) => (
            <span key={i} className="text-xs bg-gray-100 text-gray-600 rounded px-2 py-0.5">{s.a}:{s.b}</span>
          ))}
        </div>
      </div>
    );
  ...
  if (rd.type === 'basketball') {
    return (
      <div className="text-center py-2 mb-4">
        <p className="text-3xl font-bold text-gray-900 tracking-tight">{rd.scoreA} — {rd.scoreB}</p>
        <p className="text-xs text-gray-400 mt-1">pkt</p>
      </div>
    );
  }
  if (rd.type === 'racing') {
    return (
      <div className="py-2 mb-4">
        <p className="text-sm font-semibold text-gray-700 mb-2">Klasyfikacja:</p>
        <ol className="space-y-1">
          {rd.rankings.map((r, i) => (
            <li key={r.participantId} className="flex items-center gap-2 text-sm">
              <span className="w-5 h-5 rounded-full bg-gray-100 text-gray-600 flex items-center justify-center text-xs font-bold">{i + 1}</span>
              <span className="text-gray-800">{r.participantId}</span>
              {r.lapTime && <span className="text-gray-400 text-xs">{r.lapTime}</span>}
            </li>
          ))}
        </ol>
      </div>
  ...
  if (rd.type === 'generic') {
    return (
      <div className="py-2 mb-4 text-center">
        <p className="text-lg font-semibold text-gray-900">{rd.text}</p>
        {rd.winner && <p className="text-xs text-gray-500 mt-1">Zwycięzca: Drużyna {rd.winner}</p>}
      </div>
    );
  }
  return null;
  ...
  function Stepper({ value, onChange }: { value: number; onChange: (v: number) => void }) {
    return (
      <div className="flex items-center gap-1.5">
        <button type="button" onClick={() => onChange(Math.max(0, value - 1))} className="w-6 h-6 rounded-full border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-gray-50 text-sm">
          <Minus className="w-3 h-3" />
        </button>
        <span className="w-5 text-center text-sm font-semibold">{value}</span>
        <button type="button" onClick={() => onChange(value + 1)} className="w-6 h-6 rounded-full border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-gray-50 text-sm">
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
      ...
      {!initialResult && !isOrganizer && (
        <p className="text-sm text-gray-400 text-center py-3">Wynik nie został jeszcze wpisany.</p>
      )}
      ...
      {isOrganizer && (
        <div className="space-y-5">
          ...
          {family === 'goals' && (
            <>
              <div>
                <p className="text-sm font-medium text-gray-700 mb-2">Wynik końcowy</p>
                <div className="flex items-center gap-3">
                  <input type="number" min={0} max={99} value={scoreA} onChange={(e) => setScoreA(e.target.value)}
                    className="w-16 text-center border border-gray-300 rounded-lg px-2 py-2 text-xl font-bold focus:outline-none focus:ring-2 focus:ring-green-700" placeholder="A" />
                  <span className="text-gray-400 font-bold text-xl">—</span>
                  <input type="number" min={0} max={99} value={scoreB} onChange={(e) => setScoreB(e.target.value)}
                    className="w-16 text-center border border-gray-300 rounded-lg px-2 py-2 text-xl font-bold focus:outline-none focus:ring-2 focus:ring-green-700" placeholder="B" />
                </div>
              </div>

              {regulars.length > 0 && (
                <div>
                  <p className="text-sm font-medium text-gray-700 mb-2">Strzelcy (gole + asysty)</p>
                        ...
                        <li key={p.id} className="flex items-center justify-between gap-3 py-1">
                          ...
                          <div className="flex items-center gap-3">
                            <div className="flex items-center gap-1">
                              <span className="text-xs text-gray-400">Gole</span>
                              <Stepper value={s.goals} onChange={(v) => updateStat('goals', v)} />
                            </div>
                            <div className="flex items-center gap-1">
                              <span className="text-xs text-gray-400">Asysty</span>
          ...
          {family === 'volleyball' && (
            <div>
              <div className="flex items-center justify-between mb-3">
                <p className="text-sm font-medium text-gray-700">Sety</p>
                <div className="text-2xl font-bold text-gray-900">{setsA} — {setsB}</div>
              </div>
                ...
                {sets.map((s, i) => (
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <input type="number" min={0} max={99} value={s.a}
                      ...
                      className="w-14 text-center border border-gray-300 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-green-700" />
          ...
          {family === 'basketball' && (
            <>
              <div>
                <p className="text-sm font-medium text-gray-700 mb-2">Wynik końcowy (pkt)</p>
                <div className="flex items-center gap-3">
                  <input type="number" min={0} value={basketA} onChange={(e) => setBasketA(e.target.value)}
                    className="w-20 text-center border border-gray-300 rounded-lg px-2 py-2 text-xl font-bold focus:outline-none focus:ring-2 focus:ring-green-700" placeholder="A" />
                  <span className="text-gray-400 font-bold text-xl">—</span>
                  <input type="number" min={0} value={basketB} onChange={(e) => setBasketB(e.target.value)}
                    className="w-20 text-center border border-gray-300 rounded-lg px-2 py-2 text-xl font-bold focus:outline-none focus:ring-2 focus:ring-green-700" placeholder="B" />
                </div>
              </div>
              {regulars.length > 0 && (
                <div>
                  <p className="text-sm font-medium text-gray-700 mb-2">Statystyki graczy (opcjonalnie)</p>
                  <ul className="space-y-2">
                    {regulars.map((p) => {
                      ...
                      return (
                        <li key={p.id} className="flex items-center justify-between gap-2 py-1">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-gray-400">Zbiórki</span>
            ...
            <div>
              ...
              <ul className="space-y-2">
                {rankings.map((r, i) => {
                  ...
                  return (
                    <li key={r.participantId} className="flex items-center gap-3 p-2 bg-gray-50 rounded-lg">
                      ...
                      <span className="text-sm text-gray-800 flex-1">{p?.name ?? r.participantId}</span>
          ...
          {family === 'generic' && (
            <div className="space-y-3">
              ...
              <div>
                <p className="text-sm font-medium text-gray-700 mb-1">Zwycięzca (opcjonalnie)</p>
                <div className="flex gap-2">
                  {(['A', 'B', 'remis'] as const).map((w) => (
                    <button key={w} type="button" onClick={() => setGenericWinner((prev) => prev === w ? '' : w)}
                      className={['px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors',
                        genericWinner === w ? 'bg-green-700 text-white border-green-700' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-400'
                      ].join(' ')}>
