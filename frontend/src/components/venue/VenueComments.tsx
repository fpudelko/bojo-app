...
import { getFieldComments, addFieldComment, deleteFieldComment } from '@/lib/fieldComments';
import { useAuth, displayName } from '@/lib/auth';
import { useToast } from '@/lib/toast';

/**
 * Komentarze pod obiektem z katalogu — bliźniak `EventComments`, ale osadzony
 * w innym czasie: komentarz pod boiskiem przeżywa każdy mecz na tym boisku.
 * Rzeczy, które ludzie tu piszą, to stan miejsca („bramki bez siatek", „brama
 * od parkingu zamykana po 20"), a nie ustalenia do jednego grania.
 */
export default function VenueComments({ fieldId }: { fieldId: string }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    getFieldComments(fieldId)
      .then((c) => { if (!anulowane) setComments(c); })
      .catch((e) => console.error('[VenueComments]', e))
      .finally(() => { if (!anulowane) setLoading(false); });
  ...
  const handleAdd = async () => {
    ...
    setBusy(true);
    try {
      const comment = await addFieldComment(fieldId, user.id, displayName(user), body.trim());
  ...
  const handleDelete = async (commentId: string) => {
    try {
      await deleteFieldComment(commentId);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
      ...
      <p className="mb-4 text-xs text-slate-500">
        Co warto wiedzieć przed przyjściem: stan nawierzchni, oświetlenie, dostęp, parking.
      </p>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-slate-400">
                    {format(parseISO(c.createdAt), 'd MMM yyyy', { locale: pl })}
                  </span>
                </div>
      ...
      {user ? (
        <div className="flex gap-2 pt-3 border-t border-slate-100">
          <textarea
            ...
            placeholder="Np. bramki bez siatek, wejście od strony parkingu…"
            rows={2}
            maxLength={1000}
