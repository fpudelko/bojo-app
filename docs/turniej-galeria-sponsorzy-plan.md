...
>
> **Aktualizacja 2026-09-22 (trzecia) — Etapy 7–8 zrobione, plan domknięty.**
...
> (migracja `161`).** Właściciel zdecydował się na R2 zamiast Supabase
...
> → [docs/domena.md](./domena.md#turniej-media-zdjęcia-logotypy-sponsorów-na-cloudflare-r2-161).
...
1. ~~Bucket `turniej-media` ręcznie w Supabase Dashboard (publiczny).~~
   **NIEAKTUALNE od 2026-09-23** — magazyn to teraz Cloudflare R2, migracja
   `161` skasowała polityki `storage.objects` z punktu 2. Bucket R2, jego
...
8. Podpięcie w `TurniejClient.tsx` (zakładka Info) i `PanelClient.tsx` (zakładka Ustawienia).
   **Zrobione 2026-09-22.** Dwie rzeczy dołożone ponad plan: (a) sekcje w panelu widzi
   także współorganizator z `moze_edytowac` — reszta Ustawień jest dla organizatora, ale
   RLS z `159` wpuszcza każdego z `czy_zarzadza_turniejem()`, więc interfejs nie może być
   węższy niż baza; (b) link sponsora dopuszcza wyłącznie `http`/`https`
   (`normalizujLinkSponsora()`/`bezpiecznyLinkSponsora()`) — plan o tym milczał, a
   `javascript:` w linku widocznym dla każdego to wstrzyknięcie skryptu.
