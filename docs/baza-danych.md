...
| `group_join_requests` | `150` | Prośby o dołączenie do ekipy — jedna na osobę i ekipę (`UNIQUE`), `status` `oczekuje`/`przyjeta`/`odrzucona`, `z_kodu`/`invited_by` (prośba z linku zaproszenia i jego autor). Bez polityk INSERT i UPDATE: piszą tu wyłącznie `popros_o_dolaczenie_do_grupy()`, `popros_o_dolaczenie_kodem()` i `rozpatrz_prosbe_do_grupy()` |
...
| `094_zaproszenia_do_grupy` | `group_members.invited_by` + `groups.join_code_rotated_at`. RPC `dolacz_do_grupy_kodem(kod, od)` (wówczas jedyna droga samodzielnego dołączenia — weryfikuje `od` w bazie, zanim zapisze zapraszającego; **usunięta w `150`**, bo do ekipy nie wchodzi się z linku — zastępuje ją `popros_o_dolaczenie_kodem`), `dodaj_czlonka_do_grupy` (dla `can_manage_members`, bez kodu), `odswiez_kod_grupy` (wyłącznie założyciel). **Zdejmuje politykę INSERT na `group_members`** — dotąd wystarczyło znać UUID grupy (publicznie czytelne), żeby się do niej dopisać |
...
| `096_zaproszanie_do_grupy` | `group_members.can_invite` (domyślnie `true`, jak `can_create_events` w `092` — dziś każdy widzi „Zaproś" bez bramki) — czwarty niezależny przełącznik, kto widzi przycisk „Zaproś" i kod dołączenia. Bramka wyłącznie UI: ani `dolacz_do_grupy_kodem` (`094`, usunięta w `150`), ani jej następca `popros_o_dolaczenie_kodem` nie sprawdzają uprawnień osoby, która podała kod — od `150` nie muszą, bo kod składa prośbę, a nie wpuszcza do składu. Trigger `ustaw_role_czlonka()` (`092`) przedefiniowany, żeby wymusić `can_invite = true` na founderze |
...
| `150_prywatne_ekipy` | **Ekipa przestaje być publiczną wizytówką, a wejście do niej zawsze wymaga czyjejś decyzji.** `groups` i `group_members` tracą `USING (true)` — wiersz ekipy i jej skład czyta wyłącznie członek (plus założyciel i admin). Nazwę wydaje wąska funkcja `grupa_publicznie(id)` (`id` + `name`, nic więcej), bo podgląd linku i metadane OG renderuje klucz `anon`. **Kasuje `dolacz_do_grupy_kodem()` z `094`** — kod jest jeden dla całej ekipy, a link zostaje w czacie na zawsze, więc wpuszczanie nim do składu było furtką bez klamki; dopóki funkcja żyłaby z `GRANT`-em dla `authenticated`, byłaby obejściem całej migracji. Nowa tabela `group_join_requests` + `zloz_prosbe_do_grupy()` (wspólne jądro, `REVOKE EXECUTE ... FROM PUBLIC`, bo `z_kodu`/`invited_by` nie mogą przyjść od klienta), dwa wejścia `popros_o_dolaczenie_do_grupy()` i `popros_o_dolaczenie_kodem()` (odrzucenie blokuje ponowną prośbę na 7 dni) oraz `rozpatrz_prosbe_do_grupy()`. `podglad_zaproszenia_do_grupy(kod, od)` obsługuje `/g/[kod]` — uprawnieniem jest sam kod, wynik to wizytówka (liczba osób, najbliższy termin), nie skład. Trzy nowe typy powiadomień: `prosba_do_grupy`, `prosba_do_grupy_przyjeta`, `prosba_do_grupy_odrzucona`. **Świadomie NIE rusza polityk na `events`** — mecz prywatny przypięty do ekipy zostaje czytelny dla każdego, kto ma adres, bo z linku do meczu mają grać ludzie spoza ekipy; cena (terminarz ekipy da się wylistować po `group_id`) jest opisana w `docs/domena.md § Co zostaje otwarte` i pilnowana asercją w `supabase/test/rls.sql` |
...
| `dodaj_czlonka_do_grupy`, `odswiez_kod_grupy` | Dopisanie członka przez zarządzającego i rotacja kodu przez założyciela (`SECURITY DEFINER`, `094`). Trzecia funkcja z tamtej migracji, `dolacz_do_grupy_kodem` (dołączenie kodem od ręki), **została usunięta w `150`** — do ekipy nie wchodzi się z linku; kod składa prośbę |
| `grupa_publicznie` | Nazwa ekipy dla kogoś spoza niej — `id` i `name`, nic więcej. Jedyne okno na ekipę po zamknięciu `groups` w `150` (`SECURITY DEFINER`, `GRANT` dla `anon`) |
| `zloz_prosbe_do_grupy` | Wspólne jądro obu wejść do prośby — sprawdza członkostwo, blokadę po odmowie (7 dni) i weryfikuje zapraszającego, po czym wstawia wiersz i dzwonek dla rozpatrujących. **`REVOKE EXECUTE ... FROM PUBLIC`**: `z_kodu`/`invited_by` muszą być wyliczone z prawdziwego kodu, nie przysłane przez klienta — a Postgres nadaje `EXECUTE` roli `PUBLIC` każdej nowej funkcji, więc bez `REVOKE` „wewnętrzna" funkcja jest endpointem REST-a (`SECURITY DEFINER`, `150`) |
| `popros_o_dolaczenie_do_grupy`, `popros_o_dolaczenie_kodem`, `rozpatrz_prosbe_do_grupy` | Prośba o wejście do ekipy — ze strony ekipy albo z linku/kodu (ta druga podpisuje prośbę zaproszeniem) — i decyzja o niej. Przyjęcie dopisuje wiersz do `group_members`, gdzie od `094` nie ma polityki INSERT; `invited_by` bierze autora zaproszenia, a w jego braku osobę, która przyjęła (`SECURITY DEFINER`, `150`) |
| `podglad_zaproszenia_do_grupy` | Wizytówka ekipy dla kogoś, kto ma KOD — nazwa, liczba osób, najbliższy termin, liczba rozegranych meczów, imię zapraszającego (tylko gdy naprawdę jest w ekipie). Obsługuje `/g/[kod]` po zamknięciu `groups`/`group_members`/`events` (`SECURITY DEFINER`, `GRANT` dla `anon`, `150`) |
