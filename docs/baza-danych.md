...
| `wyslij_mail_do_goscia(uczestnik, powod)` | Jedna wysyłka do gościa bez konta. Milczy przy braku konfiguracji, przy wpisie bez adresu i przy wpisie już przejętym; zapisuje ślad w `maile_goscia`, co jest jednocześnie kluczem idempotencji (`SECURITY DEFINER`, `132`) |
