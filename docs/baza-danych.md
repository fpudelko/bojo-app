...
| `group_join_requests` | `150` | Prośby o dołączenie do ekipy — jedna na osobę i ekipę (`UNIQUE`), `status` `oczekuje`/`przyjeta`/`odrzucona`. Bez polityk INSERT i UPDATE: wchodzi się wyłącznie przez `popros_o_dolaczenie_do_grupy()` i `rozpatrz_prosbe_do_grupy()` |
...
| `150_prywatne_ekipy` | **Ekipa przestaje być publiczną wizytówką.** `groups` i `group_members` tracą `USING (true)` — wiersz ekipy i jej skład czyta wyłącznie członek (plus założyciel i admin). Nazwę wydaje wąska funkcja `grupa_publicznie(id)` (`id` + `name`, nic więcej), bo podgląd linku i metadane OG renderuje klucz `anon`. Mecz PRYWATNY przypięty do ekipy przestaje być czytelny dla świata: polityka „Mecz ekipy widzi ekipa" plus `czy_widoczny_mecz()` (członek / skład / imiennie zaproszony / delegat / organizator); `event_participants` idzie za meczem. **Kasuje politykę „Join code lookup" z `041`** — jej warunek `join_code IS NOT NULL` był prawdziwy dla KAŻDEGO wiersza, a polityki SELECT sumują się przez OR, więc bez tego nowa polityka nie zmieniałaby nic (złapane przez `supabase/test/rls.sql`). Nowa tabela `group_join_requests` + `popros_o_dolaczenie_do_grupy()` (odrzucenie blokuje ponowną prośbę na 7 dni) i `rozpatrz_prosbe_do_grupy()`. `podglad_zaproszenia_do_grupy(kod, od)` obsługuje `/g/[kod]` — uprawnieniem jest sam kod, wynik to wizytówka (liczba osób, najbliższy termin), nie skład. Trzy nowe typy powiadomień: `prosba_do_grupy`, `prosba_do_grupy_przyjeta`, `prosba_do_grupy_odrzucona` |
...
| `czy_widoczny_mecz` | Warunek polityk SELECT na `events` i `event_participants` — mecz prywatny przypięty do ekipy widzi członek, skład, imiennie zaproszony, delegat i organizator (`SECURITY DEFINER`, `150`) |
| `popros_o_dolaczenie_do_grupy`, `rozpatrz_prosbe_do_grupy` | Prośba o wejście do ekipy i decyzja o niej. Przyjęcie dopisuje wiersz do `group_members`, gdzie od `094` nie ma polityki INSERT (`SECURITY DEFINER`, `150`) |
