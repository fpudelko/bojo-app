...
| `wyslij_przypomnienia()` | „Jutro grasz" dla składu i organizatora oraz „po meczu" (wynik/rozliczenie) dla organizatora, gdy jest co domknąć. Idempotentna. Cel zadania `pg_cron` `bojo-przypomnienia`; działa też wywołana ręcznie — zwraca liczbę wstawionych powiadomień. NIEDOSTĘPNA dla `anon`/`authenticated` (`SECURITY DEFINER`, `129`) |
