...
127 migracji (`001`–`132`, z lukami w numeracji — dwóch numerów tuż przed `082` brak) w
...
| `potwierdz_udzial(uczestnik, bramkarz, metoda, karta, dostawca)` | „Obserwuję” → „Gram” w JEDNEJ transakcji: sprawdza właściciela wpisu i stan meczu, woła `sync_reserve_claim`, liczy `czy_na_rezerwe()` i zapisuje. Lustro `dolacz_do_meczu()` dla ostatniej ścieżki wejścia do składu, która liczyła pojemność w przeglądarce — dwie osoby klikające „Gram” w tej samej sekundzie lądowały obie w składzie, ponad limit (`SECURITY DEFINER`, `131`) |
| `czy_zarzadza_wpisem(event_id)` | Organizator, delegat od składu lub płatności, albo admin. Używana przez `pilnuj_wlasnego_wpisu()` (`SECURITY DEFINER`, `131`) |
| `czy_moze_dopisac_goscia(event_id)` | Czy ten zalogowany może dopisać gościa: mecz ma `allow_guest_adds`, a on sam jest w składzie i nie czeka na akceptację. W funkcji, nie wprost w polityce, bo zagląda do `event_participants` — tej samej tabeli (`SECURITY DEFINER`, `131`) |
