...
- **Kolorystyka niesie stałe znaczenie w całej apce** — dwa kolory mają dziś
  ...
  - **Różowy (`pink-*`)** — zawsze i wyłącznie odniesienie do wiadomości: kropka na
    ...
    ikona wiadomości na karcie meczu/ekipy. Nigdy nic innego.
  ...
  Nowy wskaźnik/plakietka w UI ma sprawdzić, czy mieści się w jednym z tych dwóch
  znaczeń, zanim sięgnie po `pink-*`/`blue-*` — i **nie** używać ich do niczego
  innego (inny kolor niż zwykle też jest sygnałem). Przy dodawaniu nowego typu
  wskaźnika warto od razu rozważyć, czy zasługuje na własny, konsekwentnie używany
  kolor w całej apce, zamiast doraźnego wyboru per ekran.
